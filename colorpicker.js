...
Raphael.colorwheel = function(target, color_wheel_size, no_segments){
  ...
  function disable_select(target){
    $(target).css({"unselectable": "on","-moz-user-select":
"none","-webkit-user-select": "none"});
  ...
  function ondrag(start_callback, end_callback){
    drag_callbacks = [start_callback || function(){}, end_callback ||
function(){}];
    return public_methods();
  }
  ...
  function cursor_create(size){
    var set = canvas.set().push(
        ...
        canvas.circle(0, 0, size+2).attr({"stroke-width":1, stroke:"#FFF",
opacity:0.5})
    );
  ...
  function create_hue_ring(){
    var s = hue_segement_shape(),
        tri =
canvas.path(s).attr({stroke:"rgba(0,0,0,0)"}).transform("t"+(size/2)+","+padding),
    ...
    hue_ring.outline = canvas.circle(
      center,
      center,
      center-tri_size-padding).attr({"stroke":"#000",
"stroke-width":(tri_size*2)+3, opacity:0.1});
